...
- [Development guidelines](#development-guidelines)
    - [General CentralDataBase Core](#general-centraldatabase-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
CentralDataBase Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/centraldatabasepay/centraldatabase/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_centraldatabase
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_centraldatabase --log_level=test_suite
$ valgrind -v --leak-check=full src/centraldatabased -printtoconsole
```
...
# A coverage report will now be accessible at `./test_centraldatabase.coverage/index.html`.
```

**Sanitizers**

CentralDataBase Core can be compiled with various "sanitizers" enabled, which add
...
- ThreadSocketHandler : Sends/Receives data from peers on port 13432.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 13431 for connections and services them.
...
However, in open source software such as CentralDataBase Core, where everyone uses
...
pay attention to for reviewers of CentralDataBase Core code.

General CentralDataBase Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: CentralDataBase Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against CentralDataBase Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `centraldatabase-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `centraldatabase-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of CentralDataBase Core, the wallet was always
