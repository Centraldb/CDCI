...
const char * const BITCOIN_CONF_FILENAME = "centraldatabase.conf";
const char * const BITCOIN_PID_FILENAME = "centraldatabased.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "centraldatabased -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CentralDataBase
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CentralDataBase
    // Mac: ~/Library/Application Support/CentralDataBase
    // Unix: ~/.centraldatabase
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CentralDataBase";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/CentralDataBase";
#else
    // Unix
    return pathRet / ".centraldatabase";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty centraldatabase.conf if it does not excist
