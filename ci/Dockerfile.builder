...
# centraldatabase_hash
RUN git clone https://github.com/centraldatabasepay/centraldatabase_hash
RUN cd centraldatabase_hash && python3 setup.py install
...
RUN mkdir /centraldatabase-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /centraldatabase-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /centraldatabase-src
